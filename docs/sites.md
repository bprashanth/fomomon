...
```console
fetchSitesAndPrefetchImages({bool async = false})
```

Fetches the canonical site list and downloads ghost reference images. Returns a
merged list of remote sites (from S3 or cache) plus any locally-created sites.

**`async = false`** (default — used at login via `site_prefetch_screen`):
Blocks until fresh `sites.json` is fetched from S3 and all ghost images are
downloaded. Each site in the returned list has local image paths set. On network
failure, falls back to the local cache.

**`async = true`** (used on home screen):
Returns immediately from the local cache if non-empty, then fetches fresh data
and re-downloads images in the background. If the cache is empty, falls through
to the synchronous path. Intended for home-screen refresh where blocking the UI
is not acceptable.

The returned list is always the merge of remote sites and any locally-created
sites not yet promoted to S3 via `SiteSyncService`.

---

## PWA differences

On the PWA (web), site fetching and caching work the same as native but the
storage layer differs:

- **Cache**: `sites.json` is stored in `SharedPreferences` (browser
  `localStorage`) under the key `sites_cache`, rather than a file on disk. The
  cache invariant is the same: local image keys are written before the cache is
  saved, never raw S3 URLs.
- **Ghost images**: Downloaded with Cognito auth and stored in IndexedDB via
  `LocalImageStorage`. Ghost images persist across tab reloads and PWA restarts.
  In private browsing (where IDB is unavailable), they are re-fetched on each
  launch — the app continues to work but requires network for ghost overlays.
- **Offline mode**: The cache fallback works the same as native. After the first
  successful fetch, both sites and ghost images are available offline.
- **Compass heading**: Not available in browsers; the orientation overlay is
  hidden on web.
